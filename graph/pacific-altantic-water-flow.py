...
def pacificAtlantic(heights):
    ...
    def bfs(queue, can_flow):
        while queue:
            ...
            for x, y in directions:
                ...
                if 0 <= nx < nrows and 0 <= ny < ncols and not can_flow[nx][ny] and heights[nx][ny] >= heights[a][b]:
                    can_flow[nx][ny] = True
                    queue.append((nx, ny))
    ...
    result = [(i, j) for i in range(nrows) for j in range(ncols) if can_flow_pacific[i][j] and can_flow_atlantic[i][j]]
    return result
